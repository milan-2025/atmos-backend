...
const managerController = {
  ...
  getWorkloadDashboardData: async (req, res) => {
    try {
      ...
      let notResponded =
        totalWorkloads -
        (totalFeelingGood + totalGeetingBusy + totalOverwhelmed);

      let fgPercentage =
        Number(Math.toFixed(totalFeelingGood / totalWorkloads, 4)) * 100;
      let gbPercentage =
        Number(Math.toFixed(totalGeetingBusy / totalWorkloads, 4)) * 100;

      let owPercentage =
        Number(Math.toFixed(totalOverwhelmed / totalWorkloads, 4)) * 100;

      let nrPercentage =
        Number(Math.toFixed(notResponded / totalWorkloads, 4)) * 100;
